...
def s_membership(n):
    ...
    elif (basetype(container) is builtins["StringType"]) and \
            (basetype(member) is not builtins["StringType"]):
        type_error("Membership testing for '" + repr(member) + 
                   "' in '" + repr(container) + "'.", "", n[0], n[1])
...
def s_function_definition(n):
    ...
    if not returns:
        ...
        if not yields:
            type_error("The function definition of '" + n[0] +
                       "', whose return " +
...
def s_defaultlist_single(n):
    ...
    type1 = n[1].type
    type2 = n[2].type
    if (type1 != type2 and type1 is not None and type2 is not None) or \
            (type1 is None and type2 in builtins) or \
            (type2 is None and type1 in builtins):
