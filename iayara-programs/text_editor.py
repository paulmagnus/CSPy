...
class Text_editor(Text):
    ...
    def callback(self, result, *args):
        # Updating the index of the cursor for the status bar (Line/ Col)
        index = self.index("insert")
        self.statusbar.lineCol(*index.split('.'))

        # Parenthesis match for "(", "{", and "["
    ...
    def key_release(self, key):
        """ Syntax highlighting after each key_release """
    ...
    def key_control_w(self, event):       
        """ A shortcut for exiting the window """
        root.destroy() 

    def key_control_a(self, event):
        """ A shortcut for selecting alll of the code in a window (Select all) """
    ...
    def key_control_f(self, event):
        """ Open a search box to find any specific strings """
        searchbox = SearchBox(self)

    def key_control_s(self, key):
        """ A shortcut for saving a preexisting file (Save) """
        global filename
    ...
    def key_shift_control_s(self, key):
        " A shortcut for saving a new file (Save as) """
