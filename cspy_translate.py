...
def c_FILE(child, file, tabs):
    ...
    # add cs110graphics to the python search path
    script_path = re.match(r'(?P<path>.*/)[^/]+', 
                           os.path.realpath(__file__)).group('path')
    file.write(tabs + "sys.path.append(\"" + script_path + "cs110graphics\")\n")
    advance(child)

    # add lib to the python search path
    file.write(tabs + "sys.path.append(\"" + script_path + "lib\")\n")
    advance(child)
...
def c_DECLARATION_SUITE_PASS(child, file, tabs=""):
    # 0: 'pass'
...
functions = {
    ...
    "DECLARATION_SUITE_PASS"        : c_DECLARATION_SUITE_PASS,
