...
class type_obj(object):   
    ...
    def lookup_method(self, name):
        ...
        return method
        # return method + type_obj.lookup_method(self.type, name) + \
        #                 (type_obj.lookup_method(self.super, name) \
        #                  if self.super else [])
...
class signature(object):
    ...
    def parameterCmp(self, other):
        ...
        no_match = filter(lambda p : p[0] != p[1], match_params)
...
# Simple Types
builtins["TypeType"] = type_obj("TypeType", None)
builtins["type"] = type_obj("type", builtins["TypeType"])
builtins["object"] = type_obj("object", builtins["type"])
builtins["IntType"] = type_obj("IntType", builtins["type"])
builtins["int"] = type_obj("int", builtins["IntType"])
builtins["BoolType"] = type_obj("BoolType", builtins["type"])
...
builtins["FileType"] = type_obj("FileType", builtins["type"])
builtins["file"] = type_obj("file", builtins["FileType"])

# Container Types
builtins["ListType"] = type_obj("ListType", builtins["type"])
builtins["list"] = type_obj("list", builtins["ListType"], sup=builtins["object"])
builtins["TupleType"] = type_obj("TupleType", builtins["type"])
builtins["tuple"] = type_obj("tuple", builtins["TupleType"], sup=builtins["object"])
builtins["DictType"] = type_obj("DictType", builtins["type"])
builtins["dict"] = type_obj("dict", builtins["DictType"], sup=builtins["object"])
builtins["SetType"] = type_obj("SetType", builtins["type"])
...
# Function Types
builtins["FnType"] = type_obj("FnType", builtins["type"])
builtins["fn"] = type_obj("fn", builtins["FnType"])
builtins["ProcType"] = type_obj("ProcType", builtins["type"])
builtins["proc"] = type_obj("proc", builtins["ProcType"])
...
builtins["Exception"] = type_obj("Exception", 
                                 ...
                                 sup = builtins["BaseException"],
                                 methods = {
        # This constructor is added so that Exception can be
        # inherited correctly
...
def init_list(elem_type):
     ...
     methods = {    "append" : type_obj("proc", builtins["proc"], 
                    ...
                    "copy" : type_obj("fn", builtins["fn"], 
                             sig = signature(ret = type_obj("list", 
                                            builtins["list"],
                                            elem_type = [elem_type]))), 
...
def init_dict(types):
    ...
    methods =   {  "clear" : type_obj("proc", builtins["proc"], 
                   ...
                   "get" : type_obj("fn", builtins["fn"],
                           sig = signature(params = [key_type, value_type], 
                                           ret = value_type)),
                   ...
                   "items" : type_obj("fn", builtins["fn"],
                             sig = signature(ret = init_list([init_tuple(types)]))),

                   "keys" : type_obj("fn", builtins["fn"], 
                                     sig = signature(ret = init_list([key_type]))),
                   ...
                   "popitem" : type_obj("fn", builtins["fn"], 
                                        sig = signature(ret = init_tuple(types))), 
                   ...
                   "values" : type_obj("fn", builtins["fn"], 
                              sig = signature(ret = init_list([value_type]))),
...
def builtins_tolist(parameters):
    if len(parameters) != 1:
        ...
        if base not in sequencetypes:
            ...
            newlist = init_list([builtins["string"]] if basetype(p) is builtins["StringType"] else [p.elem_type[0]])
            return (type_obj("fn", builtins["fn"], 
                                sig = signature(params = parameters, 
                                                ret = newlist)), None)
...
def builtins_todict(parameters):
    ...
    # only a list, tuple, or set can be converted to a dictionary
        ...
        return (None,  "'" + repr(parameters[0]) + "' type cannot be converted " +
                    "to 'dictionary' type.'")
    else:
        ...
        if containertype is builtins["TupleType"] and \
            ...
            heterogeneous = [t != container.elem_type[0] for t in container.elem_type]
            if any(heterogeneous):
                return (None, "'" + repr(p) + "' cannot be converted to " +
                        "'list' type because it contains more than 1 " + 
                        "element type.")
...
def builtins_frzset(parameters):
    if len(parameters) > 1:
        ...
        if base not in sequencetypes:
            ...
            newset = init_frzset([builtins["string"]] if basetype(p) is builtins["StringType"] else [p.elem_type[0]])
            return (type_obj("fn", builtins["fn"], 
                                sig = signature(params = parameters, 
                                                ret = newset)), None)
...
def builtins_makeset(parameters):
    if len(parameters) > 1:
        ...
        if basetype(p) not in sequencetypes:
            ...
            # convert to set
            newset  = init_set([builtins["string"]] if basetype(p) is builtins["StringType"] else [p.elem_type[0]])
            return (type_obj("fn", builtins["fn"], 
                                sig = signature(params = parameters, 
                                                ret = newset)), None)
...
def builtins_min(parameters):
    if len(parameters) not in (1,2):
        ...
        if not numerictype:
            ...
            (None, "'min' is not defined for '" + repr(parameters[0]) + 
                    "' and '" + repr(parameters[0]) + "' type.")
...
def builtins_repr(parameters):
    if len(parameters) != 1:
        (None, "'repr' expected exactly 1 argument, but received " + 
                str(len(parameters)) + " instead.")
    else:
