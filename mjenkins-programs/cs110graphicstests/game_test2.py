...
AUTHOR:     NAME OMITTED TO COMPLY WITH FERPA GUIDELINES
...
class Board1:
    ...
    def __init__(self, window):
        self._value = 24
        self._outline = Rectangle(window, 312, 240, (174, 200))
        self._outline.set_depth(20)
        self._outline.set_fill_color('white')
        self._side1 = Rectangle(window, 144, 240, (90, 200))
        self._side1.set_depth(20)
        self._side2 = Rectangle(window, 144, 240, (258, 200))
        self._side2.set_depth(20)
        self._top1 = Polygon(window, [(18, 80), (30, 180), (42, 80)])
        self._top2 = Polygon(window, [(42, 80), (54, 180), (66, 80)])
        self._top3 = Polygon(window, [(66, 80), (78, 180), (90, 80)])
        self._top4 = Polygon(window, [(90, 80), (102, 180), (114, 80)])
        self._top5 = Polygon(window, [(114, 80), (126, 180), (138, 80)])
        self._top6 = Polygon(window, [(138, 80), (150, 180), (162, 80)])
        self._top7 = Polygon(window, [(186, 80), (198, 180), (210, 80)])
        self._top8 = Polygon(window, [(210, 80), (222, 180), (234, 80)])
        self._top9 = Polygon(window, [(234, 80), (246, 180), (258, 80)])
        self._top10 = Polygon(window, [(258, 80), (270, 180), (282, 80)])
        self._top11 = Polygon(window, [(282, 80), (294, 180), (306, 80)])
        self._top12 = Polygon(window, [(306, 80), (318, 180), (330, 80)])
        self._bottom1 = Polygon(window, [(18, 320), (30, 220), (42, 320)])
        self._bottom2 = Polygon(window, [(42, 320), (54, 220), (66, 320)])
        self._bottom3 = Polygon(window, [(66, 320), (78, 220), (90, 320)])
        self._bottom4 = Polygon(window, [(90, 320), (102, 220), (114, 320)])
        self._bottom5 = Polygon(window, [(114, 320), (126, 220), (138, 320)])
        self._bottom6 = Polygon(window, [(138, 320), (150, 220), (162, 320)])
        self._bottom7 = Polygon(window, [(186, 320), (198, 220), (210, 320)])
        self._bottom8 = Polygon(window, [(210, 320), (222, 220), (234, 320)])
        self._bottom9 = Polygon(window, [(234, 320), (246, 220), (258, 320)])
        self._bottom10 = Polygon(window, [(258, 320), (270, 220), (282, 320)])
        self._bottom11 = Polygon(window, [(282, 320), (294, 220), (306, 320)])
        self._bottom12 = Polygon(window, [(306, 320), (318, 220), (330, 320)])
    ...
    def add_to(self, win):
        ...
        for obj in self._odds:
            obj.set_fill_color('DarkBlue')
            obj.set_depth(20)
            win.add(obj)
        for obj in self._evens:
            obj.set_fill_color('khaki')
            obj.set_depth(20)
            win.add(obj)
...
class Die(EventHandler):
    ...
    def __init__(self, board, width=35, center=(20, 370), bgcolor='white', 
        ...
        self._board = board
        self._value = 6
        self._die = Rectangle(board._window, 30, 30, center)
        self._die.set_fill_color(bgcolor)
        self._die.set_border_color(fgcolor)
        self._die.set_depth(20)
        self._width = width
        self._center = center
        self._active = False
        self._pips = []
    ...
    def activate(self):
        ...
        outline"""
        self._active = True
        self._die.set_border_color('green')
        
    def deactivate(self):
        """Deactivates the dies identifiable by a red
        outline"""
        self._active = False
        self._die.set_border_color('red')
...
class BoardSpace(EventHandler):
    ...
    def __init__(self, window, center):
        ...
        EventHandler.__init__(self)
        self._center = center
        self._rect = Rectangle(window, 24, 20, center)
        self._rect.set_border_color('white')
        self._rect.set_depth(50)
        self._piece = None
...
class Pawn(EventHandler):
    ...
    def __init__(self, board, color, ident, position):
        ...
        self._piece = Circle(board._window, 10, (0, 0))
        self._piece.set_depth(5)
        self._piece.set_fill_color(color)
        self._piece.add_handler(self)
        self._location = (0, 0)
        self._active = False
    ...
    def handle_mouse_release(self, event):
        ...
        if self._active:
            self._board.report_pawn_click(self)
...
class Board:
    ...
    def __init__(self, win):
        ...
        self._boardimage = Board1(win)
        self._boardimage.add_to(win)
        ...
        for triangle in range(12):
            xpos -= 24
            for spot in range(6):
                newCell = BoardSpace(self._window, (xpos, ypos))
                newCell.add_to(win)
        ...
        for color, which in [('white', 0), ('purple', 1)]:
            ...
            if color == 'purple':
                for i in range(len(PLAYER_2)):
                    ...
                    thisPawn.move_to(PLAYER_2[i])
                    thisPawn.deactivate()
        self._update_pawn_locations()
    ...
    def find(self, pawn):
        ...
        for i in range(len(self._cells)):
            if self._cells[i].get_center() == pawn.get_center():
                self._cells[i].addPiece(pawn)
                pawn.set_position(i)
        
    def report_pawn_click(self, pawn):
        ...
        if not self._dieUsed:
            ...
            if self._current == 0:
                pos = pos + self._die1.get_value() * 6
            else:
                pos = pos - self._die1.get_value() * 6
            pos -= pos % 6
            ...
            pos = pawn.getPosition()
            if self._current == 0:
                pos = pos + self._die2.get_value() * 6
            else:
                pos = pos - self._die2.get_value() * 6
            pos -= pos % 6
            pawn.set_position(pos)
    ...
    def _update_pawn_locations(self):
        ...
        for player in self._pawns:
            for i in range(len(player)):
                ...
                player[i].move_to(theSpace.get_center())
