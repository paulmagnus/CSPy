...
def g_function_definition(n):
    ...
    try:
        ...
        type_error("The function '" + n[0] + "' with the signature '" +
                   repr(sig) + "' has already been defined.",
                   file_name, n[1], n[2])
...
def g_class_definition(n):
    ...
    if (n[1].label == "EMPTY"):
        ...
        det_type(n[1], file_name)
        sup = n[1].type
        # verify that sup is extendable
        if not sup.extendable:
            type_error("'" + sup.type_str + "' is a builtin type that cannot" +
                       " be extended by a user-defined class.", file_name, n)
        ...
        if m.label == "PROCEDURE_DEFINITION":
            ...
            if name in methods.keys():
                if (isinstance(methods[name], list)):
                    ...
                    methods[name].append(type_obj("proc", 
                                                  builtins["proc"], sig = sig))
            ...
            if name in methods.keys():
                if (isinstance(methods[name], list)):
                    ...
                    methods[name].append(type_obj("fn",
                                                  builtins["fn"],
                                                  sig = sig))
                ...
                # First overload
                    ...
                    methods[name] = [methods[name],
                                     type_obj("fn", builtins["fn"], sig = sig)]
            # Undeclared variable - add it to the class
            else:
